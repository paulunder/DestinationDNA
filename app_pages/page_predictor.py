...
model_pipeline_path = os.path.join(
    os.path.dirname(__file__), "../outputs/pipeline/model.pkl")
model_pipeline = load_pkl_file(model_pipeline_path)


def page_predictor_body():
    """
    This function generates the content for the Predictor page.
    """
    st.title("Mountain vs. Beach Preference Predictor")
    st.write(
        "Predict whether a user prefers mountains or "
        "beaches based on their characteristics."
        )
...
def get_user_input():
    ...
    income = st.number_input(
        "Income", min_value=10000, max_value=200000, step=5000)
    education_level = st.selectbox("Education Level", list(
        education_level_mapping.keys()))
    travel_frequency = st.slider("Travel Frequency per Year", 0, 12, 1)
    preferred_activities = st.selectbox("Preferred Activities", list(
        preferred_activities_mapping.keys()))
    vacation_budget = st.number_input(
        "Vacation Budget", min_value=100, max_value=20000, step=100)
    location = st.selectbox("Location", list(location_mapping.keys()))
    proximity_to_mountains = st.number_input(
        "Proximity to Mountains (km)", 0, 300, step=5)
    proximity_to_beaches = st.number_input(
        "Proximity to Beaches (km)", 0, 300, step=5)
    favorite_season = st.selectbox("Favorite Season", list(
        favorite_season_mapping.keys()))
    pets = st.selectbox("Do you have Pets?", list(pets_mapping.keys()))
    environmental_concerns = st.selectbox("Environmental Concerns", list(
        environmental_concerns_mapping.keys()))
    ...
    input_data = pd.DataFrame([{
        ...
        "Preferred_Activities": preferred_activities_mapping[
        ...
        "Environmental_Concerns": environmental_concerns_mapping[
...
def display_result(prediction):
    if prediction is not None:
        ...
        st.markdown(
            f"<h2 style='font-size: 36px;'>The person is likely to make a vacation at the <b>{preference}</b></h2>",
